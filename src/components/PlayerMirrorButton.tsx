...
export default function PlayerMirrorButton({ channelName }: { channelName: string }) {
  ...
  async function prepare() {
    ...
    try {
      ...
      const peer = new Peer(`ch-${nextCode}`, { secure: true });
      ...
      peer.on("disconnected", () => {
        if (status === "mirroring") setStatus("waiting");
      });
    } catch (err: any) {
  ...
  async function startMirroring() {
    ...
    try {
      if (!navigator.mediaDevices?.getDisplayMedia) {
        throw new Error("Screen mirroring is not supported by this browser. Try the latest Chrome or Edge.");
      }

      // Close the setup dialog before capture so the dialog itself is not mirrored.
      setOpen(false);
      ...
      setError(err?.name === "NotAllowedError"
        ? "Screen sharing was cancelled. Click Start mirroring and choose this browser tab."
        : err?.message || "Could not start TV mirroring.");
      setStatus("waiting");
    }
  }
  ...
  return <>
    <button
      type="button"
      onClick={active ? stopMirroring : prepare}
    ...
    {open && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Mirror player to TV">
      <div className="w-full max-w-md rounded-3xl border border-[#29445e] bg-[#081321] p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-black tracking-[.18em] text-green-400">TV MIRROR</p>
            <h2 className="mt-2 text-2xl font-black">Watch on your TV</h2>
            <p className="mt-2 text-sm text-slate-400">{channelName}</p>
          </div>
        ...
        {(status === "waiting" || status === "connecting") && <>
          ...
          <ol className="mt-5 space-y-2 text-sm text-slate-300">
            ...
            <li><b>2.</b> Enter the 6-digit code above.</li>
            <li><b>3.</b> Click <b>Start mirroring</b> below.</li>
            <li><b>4.</b> In the browser picker, choose <b>This tab</b> and enable tab audio if offered.</li>
          </ol>
        ...
        <p className="mt-5 text-center text-[11px] leading-5 text-slate-500">Mirroring uses your browser's screen-share permission. Your screen stream is sent peer-to-peer after the TV connects.</p>
      </div>
    </div>}
  </>;
}
