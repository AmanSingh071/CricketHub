...
export default async function Channel({params}:{params:Promise<{id:string}>}){
  ...
  return <main className="mx-auto min-h-screen max-w-6xl px-4 py-8">
    ...
    <div className="mt-8 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black tracking-[.2em] text-green-400">LIVE PLAYER</p><h1 className="mt-2 text-4xl font-black">{c.channel_name}</h1></div>{c.url&&<PlayerMirrorButton channelName={c.channel_name}/>}</div>
    ...
    {c.url&&<p className="mt-3 text-xs text-slate-500">Mirror this player to another screen using the TV button. This free option uses a direct peer-to-peer connection.</p>}
  </main>
}
