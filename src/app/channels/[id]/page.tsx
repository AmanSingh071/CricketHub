...
export default async function Channel({params}:{params:Promise<{id:string}>}){
  ...
  return <main className="mx-auto min-h-screen max-w-6xl px-4 py-8">
    ...
    <div className="mt-8 flex flex-wrap items-end justify-between gap-4">
      ...
      {c.url&&<PlayerMirrorButton channelName={c.channel_name} channelUrl={c.url}/>}
    </div>
    ...
    {c.url&&<p className="mt-3 text-xs text-slate-500">Use the TV icon above the player to send this channel to a nearby Google Cast TV or Chromecast.</p>}
  </main>
}
